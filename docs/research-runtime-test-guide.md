...
更新：2026-08-20
适用版本：包含 `research-runtime.ts` 的 Probelab 开发版
...
Job lifecycle 与叶子活动是两个维度。Runtime Dock 和 `/watch` 中的 `now:`/`last:` 只描述 command、file change、search 或 `probelab_host` 调用；叶子活动的 `completed` 不得改变 Action/job lifecycle。只有持久 job state 自身进入 `completed` 且 `finishedAt` 已记录，才表示 executor 完成。
...
1. Codex 调用 `consult_probelab` 或 App Server user-input request；
...
Host capability 不走上述自由文本咨询路径。Codex 调用 `probelab_host` 且没有匹配 grant 时：
...
如果没有 attached project-aware TUI，请求应耐久留在 inbox；恢复 TUI 后再弹窗。整个过程不要求 Leader 生成 grantId，也不应额外调用 `consult_probelab`。
...
用户仍可使用原生 `/new`。它同样会生成 ProjectView，但绕过 readiness 与 rotation request/completion 审计，因此不用于验证“Project 是否足以接管 Session”这一 Runtime 性质。Probelab 永远不会自动执行轮换。
