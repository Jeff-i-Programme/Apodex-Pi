...
The source checkout is a fast-iteration development harness. Stable npm installs keep immutable package files separate from user configuration (`~/.config/apodex_pi` by default) and runtime state (`~/.local/state/apodex_pi` by default). Neither directory belongs in this repository or in a release tarball.
