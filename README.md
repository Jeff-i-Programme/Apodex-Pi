...
本机 Windows 启动记录。Node 22、源码目录 `G:\project\Probelab`、模型 Apodex 1.1。
...
Probelab 用自己的 `.pi/agent/`，不会读 `~\.pi\agent`。把已有的 Apodex 配置拷过去：
...
Windows 上 Probelab 启动时会探测 Git Bash（Program Files、常见自定义目录、`where bash.exe`，并跳过 WSL/WindowsApps 桩），写入 `settings.json` 的 `shellPath`，并把 Git `bin` 放到 PATH 前面。也可设 `PROBELAB_SHELL`。Windows 上沙箱不可用，必须 `--full-access`。
...
科学闭环是默认能力（typed-action 提交纪律、指定产物程序循环、读数笔记本），与 Probelab 原契约同时在线，不按工作区文件夹分流。说明见 [docs/science-adapters.md](docs/science-adapters.md)。对照原版设 `PROBELAB_SCIENCE=0`。
...
（`run_apodex_pi_task.ps1` 仍是外部示例仓里的旧文件名；脚本内 `APODEX_PI_*` 会被 Probelab 当作别名读取。）

脚本会设置 `PROBELAB_DEV_MODE`、把 `D:\Git\Git\bin` 加入 PATH，并用 `--full-access` 调上面的 `bin/pi.mjs`。
