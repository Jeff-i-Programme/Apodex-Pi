...
export default function codexDelegateExtension(pi: ExtensionAPI) {
	...
	const eventContent = (job: CodexJobView): string => {
		if (job.status === "input_required" && job.pendingRequest) {
			const pending = job.pendingRequest;
			return [
				`Codex delegation ${job.id} is waiting for ${pending.audience === "user" ? "the user" : "Apodex Pi"}.`,
				`Request id: ${pending.id}`,
				`Question: ${pending.question}`,
				pending.whyBlocking ? `${job.mode === "advisor" ? "Why this matters" : "Why it blocks progress"}: ${pending.whyBlocking}` : undefined,
				pending.options?.length ? `Options: ${pending.options.join(" | ")}` : undefined,
				pending.kind === "host_capability"
					? "Apodex Pi will open the exact host-capability approval dialog in the attached TUI and return the decision to this same Codex turn."
					: undefined,
				pending.secret
	...
	const resolveHostCapabilityRequest = async (
		...
		const run = async () => {
			...
			await respondToCodexJob(job.id, {
				...
				response: approved
					? `Apodex Pi user approved host capability ${grantId}. Continue the same tool call.`
					: "Apodex Pi user denied the requested host capability. Do not retry or bypass it.",
				...ownerCheck,
			});
				...
				switch (params.action) {
					case "start": {
						...
						if (reusable && !TERMINAL_JOB_STATUSES.has(reusable.status)) {
							...
							commandReceipt = job.threadRefresh
								? `Refreshed legacy Codex thread for mission "${reusable.mission}" as ${job.id}; the same Actor now has the current Apodex Pi tools.`
					...
					case "resume": {
						...
						if (job.threadRefresh) {
							commandReceipt = `Refreshed legacy Codex thread from job ${jobId} as ${job.id}; the same mission Actor now has the current Apodex Pi tools.`;
						}
