...
export default function (pi: ExtensionAPI) {
	...
	pi.registerFlag(VARIANT_FLAG_NAME, {
		description: "V4 Pro anchor promotion context: exact (project/skills only) or research (also Probelab contract)",
		type: "string",
		default: "exact",
	});
	...
	const promote = (ctx: ExtensionContext, reason: string) => {
		...
		updateStatus(ctx);
		if (ctx.hasUI) ctx.ui.notify(`V4 Pro anchor promoted: exposing ${fullTools.length} tools and Probelab context.`, "info");
	};
	...
	pi.registerCommand("v4pro-anchor", {
		...
		handler: async (args, ctx) => {
			...
			if (action === "off") {
				...
				latestAudit = undefined;
				updateStatus(ctx);
				ctx.ui.notify("V4 Pro anchor disabled; normal Probelab prompt and tools restored.", "info");
				return;
			}
			if (action !== "status") {
