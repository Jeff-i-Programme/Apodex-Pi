...
import { probelabStateRoot } from "./runtime-paths.mjs";
import {
...
export const PROBELAB_STATE_ROOT = probelabStateRoot(HARNESS_ROOT);
export const DEFAULT_CODEX_JOB_ROOT = join(PROBELAB_STATE_ROOT, "codex", "jobs");
...
export const DEFAULT_CODEX_ADVISOR_MODEL = process.env.PROBELAB_CODEX_ADVISOR_MODEL?.trim() || "gpt-5.6-sol";
export const DEFAULT_CODEX_ADVISOR_REASONING_EFFORT = process.env.PROBELAB_CODEX_ADVISOR_EFFORT?.trim() || "max";
export const DEFAULT_CODEX_EXECUTOR_MODEL = process.env.PROBELAB_CODEX_EXECUTOR_MODEL?.trim() || "gpt-5.6-sol";
export const DEFAULT_CODEX_EXECUTOR_REASONING_EFFORT = process.env.PROBELAB_CODEX_EXECUTOR_EFFORT?.trim() || "max";
...
export const DEFAULT_CODEX_RETENTION_DAYS = Number.parseInt(process.env.PROBELAB_CODEX_RETENTION_DAYS ?? "30", 10) || 30;
export const DEFAULT_CODEX_KEEP_TERMINAL_JOBS = Number.parseInt(process.env.PROBELAB_CODEX_KEEP_TERMINAL_JOBS ?? "200", 10) || 200;
// App Server dynamic tools are fixed when a thread is created and cannot be
// added by thread/resume. Bump this whenever the Probelab dynamic tool set
...
export function buildDelegationPrompt({
	...
	const capabilityText = hostCapabilities.length > 0
		...
		: "- None. If host authority becomes necessary, call probelab_host with the exact operation; its missing-grant path pauses for a Pi TUI decision instead of handing terminal commands to the user.";
	const interaction = mode === "advisor"
		? `Probelab retains final responsibility for user intent, evidence interpretation, and research decisions; framing and hypothesis development are collaborative. Do not redefine the objective. Use consult_probelab for a concise clarification or interpretation choice when it would materially improve shared understanding; you do not need to wait until progress is completely blocked. Avoid performative or repetitive questions.`
		: `Probelab owns research framing, evidence interpretation, and the next decision. Do not broaden the objective. Use consult_probelab only when a missing research decision or user-owned fact materially blocks progress, not for implementation choices, progress, or in-project approval. If unresolved, submit a blocked outcome with the exact blocker and remaining work.`;
	const resultInstruction = mode === "advisor"
		? `Use phase=commentary for intermediate updates. When ready to hand back, call submit_probelab_result exactly once, then give a brief phase=final_answer acknowledgement. This is a continuation surface, not a verdict or review score: preserve shared understanding, candidate explanations, questions, evidence, uncertainty, and the useful next exchange.`
		: `Use phase=commentary for brief updates and continue executing; never encode a plan, preamble, checkpoint, or future intent as a result. Call submit_probelab_result exactly once only at success, a genuine blocker, or irrecoverable failure, then give a brief phase=final_answer acknowledgement. succeeded requires goal_satisfied=true and no remaining delegated work. Separate observation from interpretation and report validity limits.`;
	...
	if (continuation) {
		const continuationAuthority = fullAccess && mode !== "advisor"
			...
			: "";
		return `<probelab_continuation>
Continue the same ${mode} role, mission, and dynamic-tool protocol from this Codex thread.${continuationAuthority} Probelab still owns the objective and evidence judgment; do not reopen settled context unless freshness below requires it.
...
${resultInstruction}
</probelab_continuation>`;
	}

	const authority = fullAccess && mode !== "advisor"
		? `This executor was explicitly launched with full host access. The project remains the task scope, but it is not an OS filesystem or command sandbox: host files, commands, network, Unix sockets, credentials, and external paths are available when genuinely required by the delegated task. Do not broaden the task or expose secrets. probelab_host remains available for compatibility but is not required for ordinary host operations in this mode.`
		: `The project is the hard authority boundary. Git metadata is writable and hooks are read-only. Sandboxed tools cannot access host credentials, Unix sockets, other projects, or parent directories. Use probelab_host for an exact outside read, SSH target, or host argv; its missing-grant path pauses for the Pi TUI decision. Do not bypass the boundary, duplicate approval through consult_probelab, or hand a terminal command to the user.
...
export function buildCodexThreadRefreshNotice(previousJob, currentGit, currentResearch = {}) {
	...
	return [
		`LEGACY THREAD REFRESH: job ${previousJob.id} used Codex thread ${previousJob.threadId}, which predates Probelab dynamic-tool protocol v${CODEX_DYNAMIC_TOOL_PROTOCOL_VERSION}. A fresh Codex thread is being created so submit_probelab_result, consult_probelab, and probelab_host are all available.`,
...
export async function startCodexJob(options) {
	...
	const fullAccess = mode !== "advisor" && (options.fullAccess ?? probelabFullAccessEnabled());
