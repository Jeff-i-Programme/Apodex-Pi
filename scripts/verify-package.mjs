...
const cache = mkdtempSync(join(tmpdir(), "apodex-pi-npm-cache-"));
let output;
try {
...
assert.ok(!files.includes(".pi/config.json"), "The user-local Apodex Pi config would enter the npm package");
...
for (const required of [
	...
	".pi/schemas/apodex-pi-config.schema.json",
	".pi/themes/apodex-pi.json",
	...
	".pi/lib/project-boundary.mjs",
		...
		".pi/skills/scientific-loop/SKILL.md",
		".pi/extensions/science-loop.ts",
		".pi/extensions/tpm-pace.ts",
		".pi/lib/science-workspace.mjs",
		".pi/lib/host-shell.mjs",
		".pi/lib/tpm-pace.mjs",
		"adapters/science/cli.py",
	"LICENSE",
	"SECURITY.md",
