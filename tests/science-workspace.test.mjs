...
test("ordinary research workspace still loads the scientific-loop skill", () => {
	const temp = mkdtempSync(join(tmpdir(), "probelab-science-"));
	try {
...
test("executable-world and dataset folders get the same unified skill, not a fork", () => {
	const temp = mkdtempSync(join(tmpdir(), "probelab-science-ew-"));
	try {
		...
		assert.deepEqual(defaultScienceSkillPaths(packageRoot, { PROBELAB_SCIENCE: "1" }), [scientificLoop]);
	} finally {
		rmSync(temp, { recursive: true, force: true });
	}
});

test("PROBELAB_SCIENCE=0 is the only off switch, for comparing original Pi", () => {
	assert.deepEqual(defaultScienceSkillPaths(packageRoot, { PROBELAB_SCIENCE: "0" }), []);
	assert.equal(scienceLoopExtensionEnabled({ PROBELAB_SCIENCE: "0" }), false);
	assert.equal(scienceLoopExtensionEnabled({ PROBELAB_SCIENCE: "1" }), true);
});
