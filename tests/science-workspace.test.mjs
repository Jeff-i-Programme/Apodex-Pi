...
test("executable-world and dataset folders get the same unified skill, not a fork", () => {
	...
	try {
		...
		assert.deepEqual(defaultScienceSkillPaths(packageRoot, { RESEARCH_PI_SCIENCE: "1" }), [scientificLoop]);
	} finally {
		rmSync(temp, { recursive: true, force: true });
	}
});

test("RESEARCH_PI_SCIENCE=0 is the only off switch, for comparing original Pi", () => {
	assert.deepEqual(defaultScienceSkillPaths(packageRoot, { RESEARCH_PI_SCIENCE: "0" }), []);
	assert.equal(scienceLoopExtensionEnabled({ RESEARCH_PI_SCIENCE: "0" }), false);
	assert.equal(scienceLoopExtensionEnabled({ RESEARCH_PI_SCIENCE: "1" }), true);
});
