...
import {
	defaultProbelabConfig,
	ensureProbelabConfig,
	readProbelabConfig,
	probelabCoreSettings,
	probelabCredentialEnvironmentNames,
	probelabDeepSeekSearchEnabled,
	probelabEnvironment,
	resolveProbelabConfig,
	writeProbelabAgentConfig,
	writeProbelabConfig,
} from "../.pi/lib/research-config.mjs";

test("Probelab config owns research runtime settings, not the Leader model catalog", () => {
	const config = defaultProbelabConfig();
	...
	assert.equal(config.pi.settings.theme, "probelab");
});

test("eval TPM env turns retry back on even if settings had it disabled", () => {
	const previousGap = process.env.PROBELAB_TPM_GAP_MS;
	process.env.PROBELAB_TPM_GAP_MS = "25000";
	try {
		const settings = probelabCoreSettings({
			pi: { settings: { retry: { enabled: false, maxRetries: 0, provider: { maxRetries: 0 } } } },
			codex: defaultProbelabConfig().codex,
			research: defaultProbelabConfig().research,
			ui: defaultProbelabConfig().ui,
			diagnostics: defaultProbelabConfig().diagnostics,
			resources: {},
			version: 2,
		...
		if (previousGap === undefined) delete process.env.PROBELAB_TPM_GAP_MS;
		else process.env.PROBELAB_TPM_GAP_MS = previousGap;
	}
});

test("partial runtime config merges over defaults and rejects ambiguous or secret fields", () => {
	const config = resolveProbelabConfig({ codex: { executor: { model: "gpt-5.6-luna" } } });
	assert.equal(config.codex.executor.model, "gpt-5.6-luna");
	assert.equal(config.codex.executor.reasoningEffort, "max");
	assert.throws(() => resolveProbelabConfig({ typoSetting: true }), /Unknown Probelab config key/);
	assert.throws(() => resolveProbelabConfig({ research: { compaction: { softTokens: 500_000 } } }), /below hardTokens/);
	assert.throws(
		() => resolveProbelabConfig({ research: { compaction: { summaryTargetTokens: 20_000, summaryMaxTokens: 10_000 } } }),
		/below summaryMaxTokens/,
	);
	assert.throws(() => resolveProbelabConfig({ research: { search: { enabled: "sometimes" } } }), /auto, on, or off/);
	assert.throws(() => resolveProbelabConfig({ codex: { retention: { terminalDays: 0 } } }), /positive integer/);
	assert.throws(() => resolveProbelabConfig({ pi: { settings: { api_key: "do-not-store-here" } } }), /credential-like/);
});

test("v1 profile config migrates once into Pi native defaults and removes the curated scope", () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-config-migrate-"));
	try {
		...
		const config = ensureProbelabConfig(configPath);
		...
		writeProbelabAgentConfig(agentDir, config, { coreVersion: "0.84.2" });
...
test("normal launches preserve Pi native model scope and custom models", () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-native-models-"));
	try {
		const agentDir = join(root, "agent");
		const config = defaultProbelabConfig();
		writeProbelabAgentConfig(agentDir, config);
...
test("config persistence creates a private v2 file and schema", () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-config-"));
	try {
		const configPath = join(root, "config.json");
		const config = ensureProbelabConfig(configPath);
		assert.equal(config.version, 2);
		assert.equal(statSync(configPath).mode & 0o777, 0o600);
		assert.ok(statSync(join(root, "schemas", "probelab-config.schema.json")).isFile());
		const changed = writeProbelabConfig(configPath, { ...config, ui: { ...config.ui, density: "compact" } });
		assert.equal(readProbelabConfig(configPath).ui.density, "compact");
...
test("config exports runtime environment without a second Leader model selection", () => {
	const environment = probelabEnvironment(defaultProbelabConfig());
	assert.equal(Object.hasOwn(environment, "PROBELAB_ACTIVE_PROFILE"), false);
	assert.equal(environment.PROBELAB_CODEX_ADVISOR_MODEL, "gpt-5.6-sol");
	assert.equal(environment.PROBELAB_CODEX_RETENTION_DAYS, "30");
	assert.equal(environment.PROBELAB_CODEX_KEEP_TERMINAL_JOBS, "200");
	assert.equal(environment.PROBELAB_COMPACT_HARD_TOKENS, String(384 * 1024));
	assert.equal(environment.PROBELAB_SEARCH_MODEL, "deepseek-v4-flash");
	assert.equal(environment.PROBELAB_UI_DENSITY, "balanced");
});

test("legacy credential file support and native search are independent of Leader selection", () => {
	const config = defaultProbelabConfig();
	assert.deepEqual(probelabCredentialEnvironmentNames(config).sort(), ["DEEPSEEK_API_KEY", "OPENCODE_API_KEY", "ZAI_API_KEY"]);
	assert.equal(probelabDeepSeekSearchEnabled(config, { OPENCODE_API_KEY: "go-key" }), false);
	assert.equal(probelabDeepSeekSearchEnabled(config, { DEEPSEEK_API_KEY: "ds-key" }), true);
	assert.equal(probelabDeepSeekSearchEnabled(resolveProbelabConfig({ research: { search: { enabled: "off" } } }), {}), false);
	assert.throws(
		() => probelabDeepSeekSearchEnabled(resolveProbelabConfig({ research: { search: { enabled: "on" } } }), {}),
...
test("Codex, compact, and search modules consume the configured runtime environment", () => {
	...
	const result = spawnSync(process.execPath, ["--input-type=module", "-e", script], {
		...
		env: {
			...process.env,
			PROBELAB_CODEX_ADVISOR_MODEL: "gpt-advisor-configured",
			PROBELAB_CODEX_EXECUTOR_MODEL: "gpt-executor-configured",
			PROBELAB_CODEX_ADVISOR_EFFORT: "high",
			PROBELAB_CODEX_RETENTION_DAYS: "17",
			PROBELAB_CODEX_KEEP_TERMINAL_JOBS: "33",
			PROBELAB_COMPACT_SOFT_TOKENS: "111",
			PROBELAB_COMPACT_HARD_TOKENS: "222",
			PROBELAB_COMPACT_RECENT_TAIL_TOKENS: "7,8",
			PROBELAB_COMPACT_SUMMARY_TARGET_TOKENS: "9",
			PROBELAB_COMPACT_SUMMARY_MAX_TOKENS: "18",
			PROBELAB_SEARCH_MAX_SOURCES: "2",
		},
	});
...
test("/config keeps Probelab themes while exposing Pi Core model commands", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-config-ui-"));
	const previousPath = process.env.PROBELAB_CONFIG_FILE;
	try {
		const configPath = join(root, "config.json");
		process.env.PROBELAB_CONFIG_FILE = configPath;
		ensureProbelabConfig(configPath);
		...
		const ctx = {
			hasUI: true,
			ui: {
				getAllThemes: () => ["probelab", "research-graphite", "research-ember", "dark", "light"].map((name) => ({ name })),
		...
		assert.equal(readProbelabConfig(configPath).pi.settings.theme, "research-graphite");
		await commands.get("config").handler("use old-profile", ctx);
		assert.match(notices.at(-1), /Usage:/);
	} finally {
		if (previousPath === undefined) delete process.env.PROBELAB_CONFIG_FILE;
		else process.env.PROBELAB_CONFIG_FILE = previousPath;
		rmSync(root, { recursive: true, force: true });
	}
	assert.equal(compactConfigPath("/Users/polaris/Documents/Utils/Pi/.pi/config.json"), "…/Utils/Pi/.pi/config.json");
	const themes = themeSelectItems(defaultProbelabConfig());
