...
test("Probelab suppresses only the newest Codex internal log database and can restore diagnostics", () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-sqlite-"));
	try {
...
test("Codex SQLite logs are suppressed by default and require an explicit diagnostic opt-in", () => {
	...
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-sqlite-empty-"));
	try {
