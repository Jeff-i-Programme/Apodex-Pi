...
test("full access bypasses direct-path approval for Leader but keeps the mode explicit in prompt and UI", async () => {
	...
	try {
		...
		assert.match(injected.systemPrompt, /explicitly launched Apodex Pi with full access/);
	} finally {
...
test("project boundary reuses an approved external-read grant", async () => {
	const root = mkdtempSync(join(tmpdir(), "apodex-pi-boundary-external-read-"));
...
test("path resolution accepts project paths and detects traversal plus symlink escapes", async () => {
	const parent = mkdtempSync(join(tmpdir(), "apodex-pi-boundary-path-"));
	try {
...
test("project-local Apodex Pi state is idempotently excluded through shared Git metadata", async () => {
	const root = mkdtempSync(join(tmpdir(), "apodex-pi-local-exclude-"));
	try {
		...
		execFileSync("git", ["-C", main, "-c", "user.name=Apodex Pi Test", "-c", "user.email=test@apodex-pi.invalid", "commit", "-q", "-m", "fixture"]);
...
test("Codex executable resolution pins the canonical target behind PATH symlinks", async () => {
	const root = mkdtempSync(join(tmpdir(), "apodex-pi-codex-bin-"));
	try {
...
test("credential-like project paths require the same one-shot gate", async () => {
	const root = mkdtempSync(join(tmpdir(), "apodex-pi-boundary-secret-"));
	try {
		...
		assert.equal((await resolveBoundaryPath(root, ".config/apodex-pi/credentials.env")).sensitive, true);
...
test("Analysis shell keeps the project read-only with only Runtime temp writable", () => {
	const root = "/Users/example/research-project";
	const runtimeTmp = `${root}/.git/apodex-pi/tmp`;
...
test("Codex executor profile keeps project and Git writable with public network", () => {
	const root = mkdtempSync(join(tmpdir(), "apodex-pi-codex-profile-"));
	try {
		mkdirSync(join(root, ".git"));
		...
		const args = codexPermissionConfigArguments("executor", root, `${root}/.git/apodex-pi/tmp`, {
			name: "Apodex Pi",
			email: "apodex-pi@example.invalid",
		});
...
test("linked worktrees grant their real Git dir and common dir without making hooks writable", () => {
	const root = mkdtempSync(join(tmpdir(), "apodex-pi-linked-worktree-"));
	try {
...
test("shared system runtime policy is read-only in both Pi and Codex sandboxes", () => {
	const root = mkdtempSync(join(tmpdir(), "apodex-pi-runtime-policy-"));
	try {
		mkdirSync(join(root, ".git"));
		const runtimePolicy = {
			platform: "darwin",
		...
		const args = codexPermissionConfigArguments("executor", root, join(root, ".git", "apodex-pi", "tmp"), null, runtimePolicy);
