...
test("terminal Codex jobs supersede unresolved request records without changing resolved history", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-request-settlement-"));
	try {
...
test("Codex delegation exposes bounded running and terminal footer states", () => {
	...
	const runningAfterTool = formatCodexStatus({
		...
		lastActivity: { summary: "probelab_host · completed" },
	});
	assert.equal(runningAfterTool, "⚙ Codex executor 9e62a4b5 · running · last: probelab_host · completed");
...
test("Codex advisor results render as a continuation surface instead of a review verdict", () => {
	const result = {
		...
		suggested_next_exchange: "Ask Probelab which intervention is affordable first.",
	};
	const markdown = codexResultMarkdown(result);
...
test("a stale Pi Session cannot start new Codex work after Leader ownership moves", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-stale-leader-"));
	const previousRuntimeRoot = process.env.PROBELAB_RUNTIME_DIR;
	...
	try {
		...
		if (previousRuntimeRoot === undefined) delete process.env.PROBELAB_RUNTIME_DIR;
		else process.env.PROBELAB_RUNTIME_DIR = previousRuntimeRoot;
		rmSync(root, { recursive: true, force: true });
	}
});
...
if (args.includes("sandbox")) {
  process.stdout.write("probelab-codex-preflight=ok\\n");
  process.exit(0);
}
const configText = args.join(" ");
const sandbox = configText.includes("probelab_full_access") ? "${CODEX_FULL_ACCESS_PROFILE}" : configText.includes("probelab_executor") ? "${CODEX_EXECUTOR_PROFILE}" : configText.includes("probelab_advisor") ? "${CODEX_ADVISOR_PROFILE}" : "unknown";
let model = "unknown";
let isResume = false;
let activeTurn = null;
let completionTimer;
...
const complete = (prompt, leaderResponse = "") => {
  ...
  send({ id: "server-submit-result-1", method: "item/tool/call", params: { threadId: "thread-fake-123", turnId: activeTurn, callId: "submit-call-1", tool: "submit_probelab_result", arguments: result } });
};
...
createInterface({ input: process.stdin }).on("line", (line) => {
  ...
  if (message.method === "initialize") {
    ...
    hostToolPresent = message.params.dynamicTools?.some((tool) => tool.name === "probelab_host") ?? false;
    const consultation = message.params.dynamicTools?.find((tool) => tool.name === "consult_probelab");
    const submission = message.params.dynamicTools?.find((tool) => tool.name === "submit_probelab_result");
    ...
    if (prompt.includes("ask the leader live")) {
      send({ id: "server-question-1", method: "item/tool/call", params: { threadId: "thread-fake-123", turnId: activeTurn, callId: "call-1", tool: "consult_probelab", arguments: { audience: "leader", question: "Choose H1 or H2", why_blocking: "The experiment differs", options: ["H1", "H2"] } } });
    } else if (prompt.includes("HOST_READ_PATH=")) {
      const path = prompt.match(/HOST_READ_PATH=([^\\n<]+)/)?.[1]?.trim();
      send({ id: "server-host-read-1", method: "item/tool/call", params: { threadId: "thread-fake-123", turnId: activeTurn, callId: "host-call-1", tool: "probelab_host", arguments: { action: "read", path } } });
      ...
      send({ id: "server-host-command-1", method: "item/tool/call", params: { threadId: "thread-fake-123", turnId: activeTurn, callId: "host-command-1", tool: "probelab_host", arguments: { action: "command", argv: [process.execPath, path, "from-codex"], ...(grantId ? { grantId } : { cwd: process.cwd() }) } } });
    } else {
...
test("delegation prompt makes advisor collaborative while preserving executor authority", () => {
	...
	assert.match(advisor, /read-only research advisor collaborating with Probelab/);
	...
	assert.match(executor, /probelab_host/);
...
test("worker liveness is bound to an instance nonce rather than PID alone", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-worker-identity-"));
	try {
...
test("advisor, executor, and explicit resume produce durable structured jobs", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-"));
	try {
		const workspace = join(root, "workspace");
		...
		assert.equal(executor.resultSource, "submit_probelab_result");
		...
		assert.equal(refreshed.resultSource, "submit_probelab_result");
...
test("submitted executor result cannot be replaced by a later commentary item", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-phase-result-"));
	try {
		const workspace = join(root, "workspace");
		const jobRoot = join(root, "codex", "jobs");
		mkdirSync(workspace, { recursive: true });
		...
		assert.equal(completed.resultSource, "submit_probelab_result");
	} finally {
		rmSync(root, { recursive: true, force: true });
	}
});
...
test("legacy completed plus goal_satisfied false normalizes to a partial outcome", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-legacy-outcome-"));
	try {
		const workspace = join(root, "workspace");
		const jobRoot = join(root, "codex", "jobs");
		mkdirSync(workspace, { recursive: true });
...
test("executor crash after the durable side-effect barrier requires explicit reconciliation", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-unknown-"));
	try {
		const workspace = join(root, "workspace");
		const jobRoot = join(root, "codex", "jobs");
		mkdirSync(workspace, { recursive: true });
...
test("unrelated app-server threads cannot replace or complete the owned Codex turn", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-thread-scope-"));
	try {
		const workspace = join(root, "workspace");
		const jobRoot = join(root, "codex", "jobs");
		mkdirSync(workspace, { recursive: true });
...
test("mission routing reuses only the same mode and workspace", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-mission-"));
	try {
		const workspace = join(root, "workspace");
...
test("automatic Codex reuse stays on one research route while explicit continuation crosses with a warning", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-route-"));
	try {
		const workspace = join(root, "workspace");
		const jobRoot = join(root, "codex", "jobs");
		mkdirSync(workspace, { recursive: true });
...
test("sibling branches in one Pi session cannot observe or reuse each other's Codex jobs", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-branch-owner-"));
	try {
		const workspace = join(root, "workspace");
		const jobRoot = join(root, "codex", "jobs");
		mkdirSync(workspace, { recursive: true });
...
test("project Actor ownership survives Pi session rotation without crossing workspaces", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-project-actor-"));
	try {
		const workspace = join(root, "workspace");
		const otherWorkspace = join(root, "other-workspace");
		const jobRoot = join(root, "codex", "jobs");
...
test("an active Codex Actor accepts cross-session steer and cancellation through project ownership", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-project-steer-"));
	try {
		const workspace = join(root, "workspace");
		const jobRoot = join(root, "codex", "jobs");
		mkdirSync(workspace, { recursive: true });
...
test("app-server delegation supports live leader requests and durable session reattachment", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-live-"));
	try {
		const workspace = join(root, "workspace");
		const jobRoot = join(root, "codex", "jobs");
		mkdirSync(workspace, { recursive: true });
...
test("token delta storms do not amplify job-state or default audit-log writes", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-io-"));
	try {
		const workspace = join(root, "workspace");
		const jobRoot = join(root, "codex", "jobs");
		mkdirSync(workspace, { recursive: true });
...
test("App Server objective command and subagent events reach the bounded audit projection", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-observe-"));
	try {
		const workspace = join(root, "workspace");
		const jobRoot = join(root, "codex", "jobs");
		mkdirSync(workspace, { recursive: true });
...
test("parallel objective activities remain separately visible in live job state", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-parallel-ui-"));
	try {
		const workspace = join(root, "workspace");
		const jobRoot = join(root, "codex", "jobs");
		mkdirSync(workspace, { recursive: true });
...
test("Codex uses the same opaque session host-capability ledger", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-host-"));
	try {
		const workspace = join(root, "workspace");
...
test("Codex executor reuses a project-trusted host-command prefix", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-host-command-"));
	try {
		const workspace = join(root, "workspace");
		const jobRoot = join(root, "codex", "jobs");
		mkdirSync(join(workspace, ".git"), { recursive: true });
...
test("a missing Codex host grant becomes structured input and resumes the same tool call after approval", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-host-approval-"));
	try {
		const workspace = join(root, "workspace");
		const jobRoot = join(root, "codex", "jobs");
		mkdirSync(join(workspace, ".git"), { recursive: true });
...
test("a workspace has one writer lease and cancellation preserves the side-effect boundary", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-lock-"));
	try {
		const workspace = join(root, "workspace");
		const jobRoot = join(root, "codex", "jobs");
		mkdirSync(workspace, { recursive: true });
...
test("Codex retention archives only old excess terminal jobs and exact result lookup survives", async () => {
	const root = mkdtempSync(join(tmpdir(), "probelab-codex-retention-"));
	const jobRoot = join(root, "codex", "jobs");
	const workspace = join(root, "workspace");
	try {
		mkdirSync(workspace, { recursive: true });
